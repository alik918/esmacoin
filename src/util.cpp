...
const char * const BITCOIN_CONF_FILENAME = "esmacoin.conf";
const char * const BITCOIN_PID_FILENAME = "esmacoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "esmacoin" is a composite category enabling all Esmacoin-related debug output
            if(ptrCategory->count(string("esmacoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EsmacoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EsmacoinCore
    // Mac: ~/Library/Application Support/EsmacoinCore
    // Unix: ~/.esmacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EsmacoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EsmacoinCore";
#else
    // Unix
    return pathRet / ".esmacoin";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty esmacoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override esmacoin.conf
