 ...
 *
 *   // network
 *   CEsmacoinNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.esmacoin.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CEsmacoinNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CEsmacoinNetwork(UniValue objIn)
    {
...
// // can be under: EsmacoinNetwork
// //   -- signature requirements : Key1(User)
// class CEsmacoinNetworkVariable : public CGovernanceObject
// {
// private:
